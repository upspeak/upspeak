---
name: go-style
description: Use when writing, reviewing, or refactoring Go code in the Upspeak repo (any `*.go`, `go.mod`, `go.sum`) — idiomatic Go practices and community standards for naming, error handling, interfaces, concurrency, HTTP/JSON APIs, performance, and testing. Complements the project conventions in CLAUDE.md.
metadata:
  source: github/awesome-copilot (instructions/go.instructions.md)
...
> **Upspeak precedence:** Where this guidance conflicts with `CLAUDE.md` or established patterns in `app/` and `core/`, the project conventions win. Two known Upspeak-specific overrides: (1) **comments are written in en-IN** (Indian English: "organise", "behaviour"), not US English; (2) **logging uses the package-level `slog` API** — never construct or pass a `*slog.Logger`. Upspeak also keeps modules in the repo root rather than `cmd/`/`pkg/`/`internal/`.
...
> **Upspeak override:** This repo intentionally departs from the `cmd/`/`pkg/`/`internal/` layout — modules live in the repo root (`repo/`, `filter/`, `jobs/`, …) and the single `main.go` sits at the root. Follow the existing layout; do not introduce `cmd/`/`pkg/`/`internal/`.
